...
export const ProductCard: React.FC<ProductCardProps> = ({
  ...
  const primarySource = product.sources?.[0]; // Use optional chaining for safety
  ...
  const renderSourceInfo = (source: any, isExpanded = false, isSecondary = false) => {
    // This function is now safe because we only call it if 'source' exists
    if (!source.isOffer) {
      return (
      ...
      <CardContent className="p-4 sm:p-5">
        <div className="flex justify-between items-start mb-3">
          <div className="flex-1 min-w-0 pr-2">
            ...
            {/* CHANGED: Added 'primarySource &&' to safely render this block */}
            {primarySource && (
        ...
        {/* CHANGED: Added 'primarySource &&' to safely render this block */}
        {!product.isArchived && primarySource && renderSourceInfo(primarySource)}
        
        {expanded && product.sources && product.sources.slice(1).map((source, index) => (
          <div key={source.id}>
