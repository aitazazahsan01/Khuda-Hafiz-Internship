...
const Calculator: React.FC<CalculatorProps> = ({ settings }) => {
  ...
  const calculatePricing = () => {
    ...
    // Platform commission
    const platformFee = (totalPurchaseCost * settings.platformCommission) / 100;
    
    // VAT calculation
    const vatAmount = (totalPurchaseCost * settings.vatRate) / 100;
    
    // Shipping calculation
    let shippingCost = 0;
    if (totalPurchaseCost < settings.shippingThreshold) {
      shippingCost = settings.shippingFee;
    } else {
      shippingCost = settings.fullShippingCost;
    }
    
    // Creator commission
    const creatorFee = (totalPurchaseCost * settings.creatorCommission) / 100;
    
    // Total cost
    const totalCost = totalPurchaseCost + platformFee + vatAmount + shippingCost + creatorFee;
    
    // Suggested selling price (with 20% profit margin)
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  ...
                  <Badge variant="secondary">{settings.platformCommission}%</Badge>
                </div>
                <div className="flex justify-between">
                  <span>VAT Rate:</span>
                  <Badge variant="secondary">{settings.vatRate}%</Badge>
                </div>
                <div className="flex justify-between">
                  <span>Shipping Threshold:</span>
                  <Badge variant="secondary">£{settings.shippingThreshold}</Badge>
                </div>
                <div className="flex justify-between">
                  <span>Shipping Fee:</span>
                  <Badge variant="secondary">£{settings.shippingFee}</Badge>
                </div>
                <div className="flex justify-between">
                  <span>Creator Commission:</span>
                  <Badge variant="secondary">{settings.creatorCommission}%</Badge>
                </div>
              </div>
            </div>
          </div>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="space-y-2 text-sm">
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className="font-medium">£{results.creatorFee.toFixed(2)}</span>
                      </div>
                      <Separator />
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="space-y-2 text-sm">
                      <div className="flex justify-between">
                        ...
                        <span className="font-semibold text-green-600">£{results.suggestedPrice.toFixed(2)}</span>
                      </div>
                      <div className="flex justify-between">
                        <span>Profit Margin:</span>
                        <span className="font-semibold text-green-600">£{results.profitMargin.toFixed(2)}</span>
                      </div>
                      <div className="flex justify-between">
                        <span>Profit %:</span>
                        <span className="font-semibold text-green-600">
                          {((results.profitMargin / results.totalCost) * 100).toFixed(1)}%
                        </span>
                      </div>
                    </div>
                  </div>
